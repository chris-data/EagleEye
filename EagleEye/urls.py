...
from . import views, view_ext, view_pkg, views_check
...
pageurl = [
    ...
    # traffic page
    url(r'^realtime/traffic/$', views.get_traffic_detail, name='pv_detail'),
    # order page
    url(r'^realtime/order/$', views.get_order_detail, name='order_detail'),
    # pageid list
    url(r'^ajax/pageid/$', views.get_pageid, name='pageid_list'),

    url(r'^realtime/uaanalysis/$', views.get_ua_analysis, name='ua_analysis'),
...
bookingurl = [
    url(
        ...
        views.get_booking_interval, name='booking_interval'),
    url(r'^ajax/update/booking/$', views.get_append_booking, name='update-booking'),
    url(r'^ajax/update/booking/(?P<channel>[a-z]+[0-9]?)/(?P<product>[a-z]+)/(?P<interval>[0-9]+)$',
        views.get_append_booking, name='update-booking-plus'),
    url(
        ...
        views.get_booking_aggregate, name='booking_aggregate'),

]
commiturl = [
    url(
        r'^ajax/init/commit/(?P<sdt>' + regex_date + ')/(?P<edt>' + regex_date + ')/(?P<channel>[a-z]+[0-9]?)/(?P<product>[a-z]+)/(?P<interval>[0-9]+)/(?P<history>False|True)$',
        views.get_commit_interval, name='init-commit-plus'),
    url(r'^ajax/update/commit/$', views.get_append_commit, name='update-commit'),
    url(r'^ajax/update/commit/(?P<channel>[a-z]+[0-9]?)/(?P<product>[a-z]+)/(?P<interval>[0-9]+)$',
        views.get_append_commit, name='update-commit-plus'),
    url(
        ...
        views.get_commit_aggregate, name='commit_aggregate'),
]

cancelurl = [
    url(r'^ajax/cancel/$', view_ext.get_cancelorders, name='cancelorder'),
    url(r'^ajax/cancelrate/$', view_ext.get_cancelrate, name='cancelrate'),
    url(r'^ajax/autocancelrate/$', view_ext.get_autocancelrate, name='autocancelrate'),
    url(r'^ajax/cancelrate/(?P<sdt>' + regex_date + ')/(?P<edt>' + regex_date + ')/(?P<interval>[0-9]+)$',
        view_ext.get_cancelrate, name='cancelrate'),
    url(r'^ajax/autocancelrate/(?P<sdt>' + regex_date + ')/(?P<edt>' + regex_date + ')/(?P<interval>[0-9]+)$',
        view_ext.get_autocancelrate, name='autocancelrate'),
]

uaurl = [
    url(r'^ajax/ua/platform/$', view_ext.get_platform, name='platform'),
    url(r'^ajax/ua/device/$', view_ext.get_device, name='device'),
    url(r'^ajax/ua/device/(?P<sdt>' + regex_date + ')/(?P<edt>' + regex_date + ')/$', view_ext.get_device,
        name='device'),
    url(r'^ajax/ua/operationsystem/$', view_ext.get_operationsystem, name='operationsystem'),
    url(r'^ajax/ua/browser/$', view_ext.get_browser, name='browser'),
    url(r'^ajax/ua/edition/$', view_ext.get_edition, name='edition'),
    url(r'^ajax/ua/operationsystem/(?P<sdt>' + regex_date + ')/(?P<edt>' + regex_date + ')/$',
        view_ext.get_operationsystem, name='operationsystem'),
    url(r'^ajax/ua/browser/(?P<sdt>' + regex_date + ')/(?P<edt>' + regex_date + ')/$', view_ext.get_browser,
        name='browser'),
    url(r'^ajax/ua/edition/(?P<sdt>' + regex_date + ')/(?P<edt>' + regex_date + ')/$', view_ext.get_edition,
        name='edition'),
    url(r'^download/(?P<sdt>' + regex_date + ')/(?P<edt>' + regex_date + ')/$', view_ext.download_ua_excel)
]
...
checkurl = [
    ...
    url(
        ...
        name='CheckAvailable'),
    url(r'^ajax/check2/(?P<params>([0-9]{4}-[0-9]{2}-[0-9]{2}|&)+((-|[0-9]|&)+))/(?P<history>False|True)/(?P<update>False|True)$',
